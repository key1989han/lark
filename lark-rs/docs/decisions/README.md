...
| [0042](0042-intern-tree-token-labels.md) | Intern `Tree.data` / `Token.type_` labels behind `&str` accessors (`Arc<str>` storage) — kills the per-token/per-node name allocation (~1.20× on the default `parse()` path); public accessor change → escalate-tier (resolves #604) | Proposed |
| [0043](0043-bindings-output-mode-taxonomy.md) | Bindings `OutputMode` (extends ADR-0029's deferred binding half) — commit the *extension mechanism* (additive C enum + string kwarg for PyO3/WASM, no FFI callback), expose `Tree` only for now, defer `SpanTree`/`Tape` to a consumer-driven trigger (which is also the C8b/C8c named-consumer gate); resolves #244 | Proposed |
