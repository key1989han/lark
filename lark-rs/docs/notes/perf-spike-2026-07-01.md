...
- **Status:** Findings note for a perf spike. The **auto-tier subset (E1/E2/E3/E5)
  lands in this PR**; the one public-surface lever (E4, `u32` positions) is
  **escalated separately** (ADR-0040 proposed, on branch
  `claude/lark-rs-perf-spike-5ew7ag`). Not policy. Wall-clock numbers are one
  shared box (4-core Xeon @ 2.80 GHz, rustc 1.94.1, release+LTO `bench` profile),
  measured **back-to-back against `master` in one session** — only same-session
...
## What ships here (E1/E2/E3/E5) — 1.44–1.96×, no public API change
...
## Escalated separately (E4) — `u32` positions

Narrowing `Token`/`Meta` position fields `usize` → `u32` shrinks `Meta`
104 → 56 B and the value-stack element `GSlot<Child>` 264 → ~168 B — a pure-memcpy
win every backend shares, and the largest single contributor to the json_large
improvement in the fuller spike. It changes **public field types**, so it is
escalate-tier and rides `performance-strategy.md` §5 ("should the default `Tree`
get cheaper?"). Proposed as **ADR-0040** with the ≥4 GiB-input truncation bound
documented; the implementation + full green suite live on branch
`claude/lark-rs-perf-spike-5ew7ag`.
...
- **Allocator:** mimalloc as the embedding binary's `#[global_allocator]` was
  worth ~1.15–1.25× in the spike — an embedder choice worth documenting for the
  PyO3/WASM bindings, not a library dependency.
